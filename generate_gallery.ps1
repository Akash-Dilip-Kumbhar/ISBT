...
$images = ""
for ($i=1; $i -le 64; $i++) {
    $images += "          <a href=`"img/gallery/img$i.jpeg`" data-lightbox=`"isbt-gallery`" data-title=`"ISBT Kumedi Gallery Image $i`" class=`"masonry-item`">`n"
    $images += "            <img src=`"img/gallery/img$i.jpeg`" loading=`"lazy`" alt=`"Gallery Image $i`" />`n"
    $images += "          </a>`n"
}

$content = $header + "`n" + $images + $footer
Set-Content -Path "Gallery.html" -Value $content -Encoding UTF8
